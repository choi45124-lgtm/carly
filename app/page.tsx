...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="site-header"><div className="container flex items-center justify-between gap-6"><a href="#home" className="brand" aria-label="Carly Cars home"><span className="brand-mark"><CarFront /></span><span>CARLY<span>CARS</span></span></a><nav className="desktop-nav" aria-label="Primary navigation"><a href="#home">Home</a><a href="#inventory">Inventory <ChevronDown /></a><a href="#financing">Loan Application</a><a href="#finder">Car Finder</a><a href="#trade">Value My Trade</a><a href="#reviews">Testimonials</a><a href="#about">About <ChevronDown /></a></nav><div className="header-actions"><a className="phone-link" href="tel:7139365512"><Phone /> <span className="hidden lg:inline">Call us</span></a><a className="button button-primary hidden sm:inline-flex" href="#inventory">Shop cars <ArrowRight /></a><button className="menu-button" aria-label={menuOpen ? 'Close menu' : 'Open menu'} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X /> : <Menu />}</button></div></div>{menuOpen && <nav className="mobile-nav" aria-label="Mobile navigation"><a href="#home" onClick={() => setMenuOpen(false)}>Home</a><a href="#inventory" onClick={() => setMenuOpen(false)}>Inventory</a><a href="#financing" onClick={() => setMenuOpen(false)}>Loan Application</a><a href="#finder" onClick={() => setMenuOpen(false)}>Car Finder</a><a href="#trade" onClick={() => setMenuOpen(false)}>Value My Trade</a><a href="#reviews" onClick={() => setMenuOpen(false)}>Testimonials</a><a href="#about" onClick={() => setMenuOpen(false)}>About</a></nav>}</header>

      <section id="home" className="hero"><div className="hero-overlay" /><div className="container hero-content"><div className="eyebrow"><span className="eyebrow-dot" /> Houston, TX · Open today until 7 PM</div><h1>A better car<br /><em>starts here.</em></h1><p>Quality pre-owned vehicles, transparent pricing, and a team that treats you like family. Find your next ride without the dealership runaround.</p><div className="hero-actions"><a className="button button-primary button-large" href="#inventory">Browse inventory <ArrowRight /></a><a className="button button-ghost button-large" href="#financing">Get pre-approved <ChevronRight /></a></div><div className="hero-proof"><div className="avatar-stack"><span>MR</span><span>DT</span><span>JL</span></div><div><div className="stars">★★★★★</div><strong>4.8 on Google</strong><span> · 200+ happy drivers</span></div></div></div><div className="hero-stamp"><span>HOUSTON</span><strong>CARLY<br />CARS</strong><small>Drive happy.</small></div></section>
      ...
      <footer className="footer"><div className="container footer-top"><a href="#home" className="brand brand-light"><span className="brand-mark"><CarFront /></span><span>CARLY<span>CARS</span></span></a><p>Quality cars. Honest deals.<br />A better way to buy.</p><div className="footer-links"><a href="#inventory">Inventory</a><a href="#financing">Financing</a><a href="#trade">Value my trade</a><a href="#contact">Contact</a></div><div className="socials"><a href="#contact" aria-label="Facebook"><Link /></a><a href="#contact" aria-label="Instagram"><Link /></a><a href="#contact" aria-label="Youtube"><Link /></a></div></div><div className="container footer-bottom"><span>© 2026 Carly Cars. All rights reserved.</span><span>Houston, Texas · Dealer license available upon request</span></div></footer><a className="mobile-call" href="tel:7139365512"><Phone /> Call Carly Cars</a>
    </main>
  )
}
