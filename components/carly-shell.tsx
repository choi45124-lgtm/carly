...
export function CarlyHeader() {
  ...
  return <>
    <div className="topbar"><div className="container flex items-center justify-between gap-4"><span className="hidden sm:inline">Houston&apos;s trusted independent dealer since 2018</span><a href="tel:5551234567"><Phone /> (555) 123-4567</a><span className="hidden md:inline">123 Main Street · Houston, TX 77002</span></div></div>
    <header className="site-header"><div className="container flex items-center justify-between gap-6"><a href="/" className="brand" aria-label="Rodeo Autos home"><span className="brand-mark"><img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-lENOmXk0KIozPzpbNSwn6aj81HWvGI.png" alt="Rodeo Autos logo" /></span><span>RODEO<br /><small>AUTOS</small></span></a><nav className="desktop-nav" aria-label="Primary navigation">{navItems.map(([label, href]) => label === 'Inventory' ? <div className="nav-dropdown" key={href}><button className="nav-dropdown-trigger" aria-expanded={inventoryOpen} onClick={() => setInventoryOpen(!inventoryOpen)}>{label}<ChevronDown /></button>{inventoryOpen && <div className="nav-dropdown-menu" role="menu">{inventoryLinks.map(([item, itemHref]) => <a key={itemHref} href={itemHref} role="menuitem" onClick={() => setInventoryOpen(false)}>{item}<span>›</span></a>)}</div>}</div> : <a key={href} href={href}>{label}</a>)}<a href="/#about">About <ChevronDown /></a></nav><div className="header-actions"><a className="phone-link" href="tel:5551234567"><Phone /><span className="hidden lg:inline">Call us</span></a><a className="button button-primary hidden sm:inline-flex" href="/cars-for-sale">Shop cars</a><button className="menu-button" aria-label={open ? 'Close menu' : 'Open menu'} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button></div></div>{open && <nav className="mobile-nav" aria-label="Mobile navigation">{navItems.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}<a href="/#about" onClick={() => setOpen(false)}>About</a></nav>}</header>
  </>
}

export function CarlyFooter() {
  return <footer className="footer"><div className="container footer-top"><a href="/" className="brand brand-light" aria-label="Rodeo Autos home"><span className="brand-mark"><img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-lENOmXk0KIozPzpbNSwn6aj81HWvGI.png" alt="Rodeo Autos logo" /></span><span>RODEO<br /><small>AUTOS</small></span></a><p>Quality cars. Honest deals.<br />A better way to buy.</p><div className="footer-links">{navItems.slice(1, 5).map(([label, href]) => <a key={href} href={href}>{label}</a>)}<a href="/#contact">Contact</a></div></div><div className="container footer-bottom"><span>© 2026 Auto. All rights reserved.</span><span>Houston, Texas · 123 Main Street, Houston, TX 77002</span></div></footer>
}
...
export function LeadForm({ button = 'Submit request' }: { button?: string }) {
  ...
  if (sent) return <div className="success-panel"><strong>Thanks — your request is in.</strong><span>A Auto specialist will be in touch shortly.</span></div>
