...
export function CarlyHeader() {
  ...
  const aboutLinks = [['About Us', '/about-us'], ['Rodeo Auto Merch', 'https://pauto43912.vercel.app/rodeo-auto-merch']]
  return <>
    <div className="topbar"><div className="container flex items-center justify-between gap-4"><span className="hidden sm:inline">Your trusted independent dealer</span><a href="tel:5550147826"><Phone /> (555) 014-7826</a><span className="hidden md:inline">1200 Market Street · Houston, TX 77002</span></div></div>
    <header className="site-header"><div className="container flex items-center justify-between gap-6"><a href="/" className="brand" aria-label="Auto home"><span className="brand-word">Auto</span></a><nav className="desktop-nav" aria-label="Primary navigation">{navItems.map(([label, href]) => label === 'Inventory' ? <div className="nav-dropdown" key={href}><button className="nav-dropdown-trigger" aria-expanded={inventoryOpen} onClick={() => setInventoryOpen(!inventoryOpen)}>{label}<ChevronDown /></button>{inventoryOpen && <div className="nav-dropdown-menu" role="menu">{inventoryLinks.map(([item, itemHref]) => <a key={itemHref} href={itemHref} role="menuitem" onClick={() => setInventoryOpen(false)}>{item}<span>›</span></a>)}</div>}</div> : label === 'About Us' ? <div className="nav-dropdown" key={href}><button className="nav-dropdown-trigger" aria-expanded={aboutOpen} onClick={() => setAboutOpen(!aboutOpen)}>{label}<ChevronDown /></button>{aboutOpen && <div className="nav-dropdown-menu" role="menu">{aboutLinks.map(([item, itemHref]) => <a key={itemHref} href={itemHref} role="menuitem" onClick={() => setAboutOpen(false)}>{item}<span>›</span></a>)}</div>}</div> : label === 'Make A Payment' ? <a key={href} href="/payment">{label}</a> : <a key={href} href={href}>{label}</a>)}</nav><div className="header-actions"><a className="phone-link" href="tel:5550147826"><Phone /><span className="hidden lg:inline">Call us</span></a><a className="button button-primary hidden sm:inline-flex" href="/cars-for-sale">Shop cars</a><button className="menu-button" aria-label={open ? 'Close menu' : 'Open menu'} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button></div></div>{open && <nav className="mobile-nav" aria-label="Mobile navigation">{navItems.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}</nav>}</header>
  </>
}

export function CarlyFooter() {
  return <footer className="footer"><div className="container footer-top"><a href="/" className="brand brand-light"><span className="brand-word">Auto</span></a><p>Quality cars. Honest deals.<br />A better way to buy.</p><div className="footer-links">{navItems.slice(1, 5).map(([label, href]) => <a key={href} href={href}>{label}</a>)}<a href="/#contact">Contact</a><a href="/payment">Make A Payment</a></div></div><div className="container footer-bottom"><span>© 2026 Auto. All rights reserved.</span><span>Houston, Texas · 1200 Market Street, Houston, TX 77002</span></div></footer>
}
...
export function LeadForm({ button = 'Submit request' }: { button?: string }) {
  ...
  if (sent) return <div className="success-panel"><strong>Thanks — your request is in.</strong><span>An Auto specialist will be in touch shortly.</span></div>
  return <form className="lead-form" onSubmit={(e) => { e.preventDefault(); setSent(true) }}><label>Full name<input required placeholder="Your name" /></label><label>Email address<input required type="email" placeholder="you@example.com" /></label><label>Phone number<input required type="tel" placeholder="(555) 014-7826" /></label><button className="button button-primary button-full" type="submit">{button}</button><small>By submitting, you agree to be contacted by Auto.</small></form>
}
